...
export default function Register() {
    ...
    let schema = Yup.object({
        fullName: Yup.string().required('Please enter your full name').min(3,'At least 3 letters').max(30, 'At most 30 letters'),
        age: Yup.number('Numbers only!').required('Please enter your age').positive('Your age must be more than 0').integer('Years only!').min(1,'Psotive numbers only'),
        email: Yup.string().required('Please enter your valid email').email('Must be a valid email'),
        password: Yup.string().required('Please enter your password').min(6,'At least 6 characters'),
        confirmPassword: Yup.string().required("confirm your password").oneOf([Yup.ref('password')],"Not matching passwords"),
    ...
    async function CheckRegData(values){
        try{
            ...
            // let {data} = await axios.post(`http://192.168.1.27:8085${location}`,values)
            ...
            Swal.fire({
              ...
              setInterval(()=> {navigate('/login')} ,2310)
  ...
  return (
    ...
    <div className={`${style.registerEmail} d-flex`}>
        ...
        <div className={`${style.registerEmailImg} col-5`}>
            ...
            <div className={`${style.registerDetailsTitle} d-flex align-items-center justify-content-between w-100`}>
                <Link className={`${style.projectNameNav} navbar-brand`} to="/">CS<span>PP</span></Link>

                <div className='d-flex align-items-center gap-3'>
                    <h4 className='text-capitalize m-0'>already a customer?</h4>
                    <Link className={`${style.getStarted} nav-link text-capitalize`} to="/login">login</Link>
                </div>
            </div>
            ...
            <div className={`${style.registerEmailImgbottom}`}>
                <div className={`${style.registerEmailImgDesc}`}>
                    ...
                    <div className={`d-flex flex-column pt-4`}>
                        <div className={`${style.registerStep} d-flex align-items-top gap-3`}>
                                <div className='d-flex flex-column justify-content-center gap-1'>
                                    {!formik.errors.fullName && !formik.errors.email ? 
                                    (<i className={`${style.registerStepCheck} fa-regular fa-circle-check mt-1`}></i>)
                ...
                <div className={`${style.registerEmailHelp}`}>
                        ...
                        <h4>Contact us at: <Link className={`${style.registerEmailSupport}`} to='mailto:support@fuelee.com' >support@fuelee.com</Link></h4>
                </div>
            </div>
            ...
            <div className={`${style.registerDetailsForm} col-7`}>
                ...
                <div className={`${style.registerFormTitle} d-flex align-items-center justify-content-center`}>
                    ...
                    <form onSubmit={formik.handleSubmit} className=''>
                        ...
                        <div className={`${style.regform}`}>
                            ...
                            <div className='col-5'>
                                <div className={` ${style.formField} d-flex flex-column`}>
                                    ...
                                    <input type="email" placeholder='Enter your email address' name='email' value={formik.values.email} onChange={formik.handleChange} id='email' autoComplete='off'/>
                                ...
                                <div className={` ${style.formField} d-flex flex-column`}>
                                    ...
                                    <input type="password" placeholder='Enter your password' name='password' value={formik.values.password} onChange={formik.handleChange} id='password' autoComplete='off'/>
                                ...
                                <div className={` ${style.formField} d-flex flex-column`}>
                                    <label className='text-capitalize' htmlFor="confirmPassword">confirm password</label>
                                    <input type="password" placeholder='Confirm your password' name='confirmPassword' value={formik.values.confirmPassword} onChange={formik.handleChange} id='confirmPassword' autoComplete='off'/>
                                    {formik.errors.confirmPassword && formik.touched.confirmPassword ? 
                                    (<p className={`${style.errors}`}>{formik.errors.confirmPassword}</p>)
                                    : ''
                                    }
                                </div>
                            </div>
                        ...
                        <div className={`${style.registerNext} d-flex justify-content-center gap-5`}>
                        <p className='col-5'>By clicking Register, you are agree to our <Link className={`${style.registerTermsLink}`} to='/'>Terms of Service</Link> and that you have read our<Link className={`${style.registerTermsLink}`} to='/'> Privacy Policy</Link> . </p>
                        <div className='col-5'>
                        <input type='submit' value='register' className={`${style.registerNextLink} nav-link text-capitalize ms-auto`}/>
                        </div>
                        </div>
                        </div>
                    </form>
